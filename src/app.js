...
/* ⚠ This dial is a PACE, not a duration. It scales the measured service times by
    ...
    host.innerHTML = `<div class="ctl"><div class="ctl-top">
      ...
      <div class="hint">Nobody moves in this layout, so one space carries the whole visit &mdash;
        ...
        dial &mdash; the queue for a provider and the wait to be roomed sit inside it and do not
        speed up when care does.</div></div>`;
    $("cyc").oninput = e => { S.cyc = +e.target.value; syncSpeed(m); requestRun() };
    return syncSpeed(m);
  }
  host.innerHTML = `<div class="ctl"><div class="ctl-top">
...
var SELF_HASH = "", LAST_SCORE = null, LAST_HOLD = null;
/* LAST_HOLD is the engine's measured mean space-hold, not a formula. hold_all x cyc/T_A
   ...
   because the hold also contains the provider queue and the rooming floor, and neither
   scales with this dial. Reporting the computed figure is how this label was wrong by
   ~50 min once before. Take it from the run. */   // var, not let: run() assigns it before this line is reached
